...
["dart.async", "dart:async", , P, {
  ...
  _rootHandleUncaughtError_closure: {
    ...
    call$0: function() {
      ...
      t2 = $._lastCallback;
      if (t2 == null) {
        t1 = new P._AsyncCallbackEntry(t1, null);
        $._lastCallback = t1;
        $._nextCallback = t1;
        $.get$_AsyncRun_scheduleImmediateClosure().call$1(P._asyncRunCallback$closure());
      } else {
...
["dart.dom.html", "dart:html", , W, {
  ...
  CanvasRenderingContext2D: {
    "^": "CanvasRenderingContext;fillStyle},globalAlpha}",
...
["", "main.dart", , F, {
  "^": "",
  main: [function() {
    var display, player, map, controls, t1, camera, loop, t2;
    display = document.getElementById("display");
    player = new F.Player(null, null, null, null, null);
    player.Player$3(15.3, -1.2, 0.9424777960769379);
    map = new F.GameMap(null, null, null, null, null);
    map.GameMap$1(32);
    controls = new F.Controls(null, null);
    controls.Controls$0();
    t1 = $.MOBILE ? 160 : 320;
    camera = new F.Camera(null, null, null, null, null, null, null, null, null);
    camera.Camera$3(display, t1, 1.2566370614359172);
    loop = new F.GameLoop(null, null);
    loop.lastTime = 0;
    map.randomize$0();
    loop.callback = new F.main_closure(player, map, controls, camera);
    t1 = window;
    t2 = loop.get$frame();
  ...
  Controls: {
    ...
    keyDown$1: [function(e) {
      var state;
      if (this.codes.containsKey$1(J.get$keyCode$x(e))) {
        state = this.codes.$index(0, e.keyCode);
        this.states.$indexSet(0, state, true);
      }
    }, "call$1", "get$keyDown", 2, 0, 13],
    keyUp$1: [function(e) {
      var state;
      if (this.codes.containsKey$1(J.get$keyCode$x(e))) {
        state = this.codes.$index(0, e.keyCode);
        this.states.$indexSet(0, state, false);
      }
    }, "call$1", "get$keyUp", 2, 0, 13],
    Controls$0: function() {
      this.codes = P.LinkedHashMap_LinkedHashMap$_literal([37, "left", 39, "right", 38, "forward", 40, "backward"], null, null);
      this.states = P.LinkedHashMap_LinkedHashMap$_literal(["left", false, "right", false, "forward", false, "backward", false], null, null);
      C.HtmlDocument_methods.addEventListener$3(document, "keydown", this.get$keyDown(), false);
      C.HtmlDocument_methods.addEventListener$3(document, "keyup", this.get$keyUp(), false);
    }
  },
  Bitmap: {
    "^": "Object;width,height,image",
    Bitmap$3: function(src, width, height) {
    ...
    static: {Bitmap$: function(src, width, height) {
        var t1 = new F.Bitmap(null, null, null);
        t1.Bitmap$3(src, width, height);
        return t1;
      }}
  },
  Player: {
    "^": "Object;x,y,direction,weapon,paces",
    rotate$1: function(_, angle) {
      this.direction = C.JSNumber_methods.$mod(this.direction + angle + 6.283185307179586, 6.283185307179586);
    },
    walk$2: function(distance, map) {
      var t1, dx, dy;
      t1 = this.direction;
      dx = Math.cos(t1) * distance;
      t1 = this.direction;
      dy = Math.sin(t1) * distance;
      if (map.get$2(this.x + dx, this.y) <= 0)
        this.x += dx;
      if (map.get$2(this.x, this.y + dy) <= 0)
        this.y += dy;
      this.paces += distance;
    },
    Player$3: function(x, y, direction) {
      this.x = x;
      this.y = y;
      this.direction = direction;
      this.weapon = F.Bitmap$("knife_hand.png", 319, 320);
      this.paces = 0;
    }
  },
  GameMap: {
    "^": "Object;size,wallGrid,skybox,wallTexture,light",
    get$2: function(x, y) {
      var t1, t2;
      x = J.floor$0$n(x);
      y = J.floor$0$n(y);
    ...
    randomize$0: function() {
      var i, t1, t2;
      for (i = 0; t1 = this.size, i < t1 * t1; ++i) {
        t1 = this.wallGrid;
        t2 = C.C__JSRandom.nextDouble$0() < 0.3 ? 1 : 0;
        if (i >= t1.length)
          return H.ioore(t1, i);
        t1[i] = t2;
      }
    },
    cast$3: function(point, angle, range) {
      var sinv, cosv;
      sinv = Math.sin(angle);
      cosv = Math.cos(angle);
      return new F.GameMap_cast_ray(range, sinv, cosv, new F.GameMap_cast_step(P.LinkedHashMap_LinkedHashMap$_literal(["length2", 1 / 0], null, null)), new F.GameMap_cast_inspect(this, sinv, cosv)).call$1(P.LinkedHashMap_LinkedHashMap$_literal(["x", point.x, "y", point.y, "height", 0, "distance", 0], null, null));
    },
    GameMap$1: function(size) {
      this.size = size;
      this.wallGrid = new Uint8Array(size * size);
      this.skybox = F.Bitmap$("deathvalley_panorama.jpg", 4000, 1290);
      this.wallTexture = F.Bitmap$("wall_texture.jpg", 1024, 1024);
      this.light = 0;
    }
  },
  GameMap_cast_step: {
    "^": "Closure:14;noWall_0",
    call$5: function(rise, run, x, y, inverted) {
      var t1, dx, dy, t2;
      P.LinkedHashMap_LinkedHashMap(null, null, null, null, null);
      if (run === 0)
        return this.noWall_0;
      t1 = J.getInterceptor$ns(x);
      if (run > 0) {
        t1 = J.floor$0$n(t1.$add(x, 1));
        if (typeof x !== "number")
          return H.iae(x);
        dx = t1 - x;
      } else {
        t1 = J.ceil$0$n(t1.$sub(x, 1));
        if (typeof x !== "number")
          return H.iae(x);
        dx = t1 - x;
      }
      dy = dx * (rise / run);
      t1 = inverted ? J.$add$ns(y, dy) : J.$add$ns(x, dx);
      t2 = inverted ? J.$add$ns(x, dx) : J.$add$ns(y, dy);
      return P.LinkedHashMap_LinkedHashMap$_literal(["x", t1, "y", t2, "length2", dx * dx + dy * dy], null, null);
    }
  },
  GameMap_cast_inspect: {
    "^": "Closure:15;this_1,sinv_2,cosv_3",
    call$5: function(step, shiftX, shiftY, distance, offset) {
      var t1, dx, t2, dy, t3;
      t1 = this.cosv_3 < 0;
      dx = t1 ? shiftX : 0;
      t2 = this.sinv_2 < 0;
      dy = t2 ? shiftY : 0;
      step.$indexSet(0, "height", this.this_1.get$2(J.$sub$n(step.$index(0, "x"), dx), J.$sub$n(step.$index(0, "y"), dy)));
      t3 = step.$index(0, "length2");
      if (typeof t3 !== "number")
        H.throwExpression(P.ArgumentError$(t3));
      step.$indexSet(0, "distance", J.$add$ns(distance, Math.sqrt(t3)));
      if (shiftX !== 0)
        step.$indexSet(0, "shading", t1 ? 2 : 0);
      else
        step.$indexSet(0, "shading", t2 ? 2 : 1);
      step.$indexSet(0, "offset", offset - J.floor$0$n(offset));
      return step;
    }
  },
  GameMap_cast_ray: {
    "^": "Closure:16;range_4,sinv_5,cosv_6,step_7,inspect_8",
    call$1: function(origin) {
      var t1, t2, t3, stepX, stepY, nextStep, r;
      t1 = this.step_7;
      t2 = this.sinv_5;
      t3 = this.cosv_6;
      stepX = t1.call$5(t2, t3, origin.$index(0, "x"), origin.$index(0, "y"), false);
      stepY = t1.call$5(t3, t2, origin.$index(0, "y"), origin.$index(0, "x"), true);
      t1 = J.getInterceptor$asx(stepX);
      t2 = J.getInterceptor$asx(stepY);
      t3 = this.inspect_8;
      nextStep = J.$lt$n(t1.$index(stepX, "length2"), t2.$index(stepY, "length2")) ? t3.call$5(stepX, 1, 0, origin.$index(0, "distance"), t1.$index(stepX, "y")) : t3.call$5(stepY, 0, 1, origin.$index(0, "distance"), t2.$index(stepY, "x"));
      if (J.$gt$n(J.$index$asx(nextStep, "distance"), this.range_4))
        return [origin];
      r = [origin];
      C.JSArray_methods.addAll$1(r, this.call$1(nextStep));
      return r;
    }
  },
  Camera: {
    "^": "Object;ctx,width,height,resolution,spacing,fov,range,lightRange,scale",
    drawColumn$4: function(column, ray, angle, map) {
      var ctx, texture, left, width, hit, s, t1, step, t2, t3, t4, textureX, z, wallHeight, dict;
      ctx = this.ctx;
      texture = map.wallTexture;
      left = C.JSNumber_methods.toInt$0(Math.floor(column * this.spacing));
      width = C.JSNumber_methods.toInt$0(Math.ceil(this.spacing));
      hit = -1;
      while (true) {
        ++hit;
        if (!(hit < ray.length && J.$le$n(J.$index$asx(ray[hit], "height"), 0)))
          break;
      }
      for (s = ray.length - 1, t1 = J.getInterceptor$x(ctx); s >= 0; --s) {
        if (s >= ray.length)
          return H.ioore(ray, s);
        step = ray[s];
        t2 = C.C__JSRandom.nextDouble$0();
        Math.pow(t2, 3);
        if (s === hit) {
          t2 = texture.width;
          t3 = J.getInterceptor$asx(step);
          t4 = t3.$index(step, "offset");
          if (typeof t4 !== "number")
            return H.iae(t4);
          textureX = C.JSNumber_methods.toInt$0(Math.floor(t2 * t4));
          t4 = t3.$index(step, "height");
          t2 = t3.$index(step, "distance");
          z = J.$mul$ns(t2, Math.cos(angle));
          t2 = this.height;
          if (typeof t4 !== "number")
            return H.iae(t4);
          if (typeof z !== "number")
            return H.iae(z);
          wallHeight = t2 * t4 / z;
          dict = P.LinkedHashMap_LinkedHashMap(null, null, null, null, null);
          dict.$indexSet(0, "top", t2 / 2 * (1 + 1 / z) - wallHeight);
          dict.$indexSet(0, "height", wallHeight);
          t1.set$globalAlpha(ctx, 1);
          ctx.drawImage(texture.image, textureX, 0, 1, texture.height, left, dict.$index(0, "top"), width, dict.$index(0, "height"));
          ctx.fillStyle = "#000000";
          t3 = J.$add$ns(t3.$index(step, "distance"), t3.$index(step, "shading"));
          t2 = this.lightRange;
          if (typeof t3 !== "number")
            return t3.$div();
          ctx.globalAlpha = P.max(t3 / t2 - map.light, 0);
          ctx.fillRect(left, dict.$index(0, "top"), width, dict.$index(0, "height"));
        }
      }
    },
    drawColumns$2: function(player, map) {
      var column, t1, angle;
      J.save$0$x(this.ctx);
      for (column = 0; t1 = this.resolution, column < t1; ++column) {
        angle = this.fov * (column / t1 - 0.5);
        this.drawColumn$4(column, map.cast$3(player, player.direction + angle, this.range), angle, map);
      }
      J.restore$0$x(this.ctx);
    },
    Camera$3: function(canvas, resolution, fov) {
      var t1, t2;
      this.ctx = J.getContext$1$x(canvas, "2d");
      t1 = window.innerWidth;
      if (typeof t1 !== "number")
        return t1.$mul();
      t1 = C.JSNumber_methods.toInt$0(Math.floor(t1 * 0.5));
      canvas.width = t1;
      this.width = t1;
      ...
      t2 = this.width;
      this.spacing = t2 / resolution;
      this.fov = fov;
      this.range = $.MOBILE ? 8 : 14;
      this.lightRange = 5;
      this.scale = (t2 + t1) / 1200;
    }
  },
  GameLoop: {
    "^": "Object;callback,lastTime",
    callback$1: function(arg0) {
      return this.callback.call$1(arg0);
    },
    frame$1: [function(time) {
      var t1, seconds, t2;
      t1 = J.$sub$n(time, this.lastTime);
      if (typeof t1 !== "number")
        return t1.$div();
      seconds = t1 / 1000;
      this.lastTime = time;
      if (seconds < 0.2)
        this.callback$1(seconds);
      t1 = window;
      t2 = this.get$frame();
      C.Window_methods._ensureRequestAnimationFrame$0(t1);
      C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(t2));
    }, "call$1", "get$frame", 2, 0, 17]
  },
  main_closure: {
    "^": "Closure:18;player_0,map_1,controls_2,camera_3",
    call$1: function(seconds) {
      var t1, t2, t3, t4, t5, t6, width, left, t7, t8;
      t1 = this.map_1;
      t2 = t1.light;
      if (t2 > 0)
        t1.light = P.max(t2 - 10 * seconds, 0);
      else if (C.C__JSRandom.nextDouble$0() * 5 < seconds)
        t1.light = 2;
      t2 = this.player_0;
      t3 = this.controls_2;
      if (t3.states.$index(0, "left") === true)
        t2.rotate$1(0, -3.141592653589793 * seconds);
      if (t3.states.$index(0, "right") === true)
        t2.rotate$1(0, 3.141592653589793 * seconds);
      if (t3.states.$index(0, "forward") === true)
        t2.walk$2(3 * seconds, t1);
      if (t3.states.$index(0, "backward") === true)
        t2.walk$2(-3 * seconds, t1);
      t3 = this.camera_3;
      t4 = t2.direction;
      t5 = t1.skybox;
      t6 = t1.light;
      width = t3.width * (6.283185307179586 / t3.fov);
      left = -width * t4 / 6.283185307179586;
      J.save$0$x(t3.ctx);
      J.drawImageScaled$5$x(t3.ctx, t5.image, left, 0, width, t3.height);
      if (left < width - t3.width)
        J.drawImageScaled$5$x(t3.ctx, t5.image, left + width, 0, width, t3.height);
      if (t6 > 0) {
        J.set$fillStyle$x(t3.ctx, "#ffffff");
        J.set$globalAlpha$x(t3.ctx, t6 * 0.1);
        t4 = t3.ctx;
        t5 = t3.height * 0.5;
        J.fillRect$4$x(t4, 0, t5, t3.width, t5);
      }
      J.restore$0$x(t3.ctx);
      t3.drawColumns$2(t2, t1);
      t1 = t2.weapon;
      t2 = t2.paces;
      t4 = Math.cos(t2 * 2);
      t5 = t3.scale;
      ...
      t7 = t3.width;
      t8 = t3.height;
      J.drawImageScaled$5$x(t3.ctx, t1.image, t7 * 0.66 + t4 * t5 * 6, t8 * 0.6 + t2 * t6 * 6, t1.width * t6, t1.height * t6);
    }
  }
},
1],
...
W.KeyEvent.$isKeyEvent = true;
W.KeyEvent.$isObject = true;
...
{func: "Map__num_num_num_num_bool", ret: P.Map, args: [P.num, P.num, P.num, P.num, P.bool]},
{func: "Map__Map_num_num_num_num", ret: P.Map, args: [P.Map, P.num, P.num, P.num, P.num]},
{func: "List__Map", ret: P.List, args: [P.Map]},
...
function dart_precompiled($collectedClasses) {
  ...
  function Controls(codes, states) {
    this.codes = codes;
    this.states = states;
  }
  Controls.builtin$cls = "Controls";
  if (!"name" in Controls)
    Controls.name = "Controls";
  $desc = $collectedClasses.Controls;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Controls.prototype = $desc;
  function Bitmap(width, height, image) {
    this.width = width;
    this.height = height;
    this.image = image;
  }
  ...
  function Player(x, y, direction, weapon, paces) {
    this.x = x;
    this.y = y;
    this.direction = direction;
    this.weapon = weapon;
  ...
  function GameMap(size, wallGrid, skybox, wallTexture, light) {
    this.size = size;
  ...
  GameMap.builtin$cls = "GameMap";
  if (!"name" in GameMap)
    GameMap.name = "GameMap";
  $desc = $collectedClasses.GameMap;
  if ($desc instanceof Array)
    $desc = $desc[1];
  GameMap.prototype = $desc;
  function GameMap_cast_step(noWall_0) {
    this.noWall_0 = noWall_0;
  }
  GameMap_cast_step.builtin$cls = "GameMap_cast_step";
  if (!"name" in GameMap_cast_step)
    GameMap_cast_step.name = "GameMap_cast_step";
  $desc = $collectedClasses.GameMap_cast_step;
  if ($desc instanceof Array)
    $desc = $desc[1];
  GameMap_cast_step.prototype = $desc;
  function GameMap_cast_inspect(this_1, sinv_2, cosv_3) {
    this.this_1 = this_1;
    this.sinv_2 = sinv_2;
    this.cosv_3 = cosv_3;
  }
  GameMap_cast_inspect.builtin$cls = "GameMap_cast_inspect";
  if (!"name" in GameMap_cast_inspect)
    GameMap_cast_inspect.name = "GameMap_cast_inspect";
  $desc = $collectedClasses.GameMap_cast_inspect;
  if ($desc instanceof Array)
    $desc = $desc[1];
  GameMap_cast_inspect.prototype = $desc;
  function GameMap_cast_ray(range_4, sinv_5, cosv_6, step_7, inspect_8) {
    this.range_4 = range_4;
    this.sinv_5 = sinv_5;
    this.cosv_6 = cosv_6;
    this.step_7 = step_7;
    this.inspect_8 = inspect_8;
  }
  GameMap_cast_ray.builtin$cls = "GameMap_cast_ray";
  if (!"name" in GameMap_cast_ray)
    GameMap_cast_ray.name = "GameMap_cast_ray";
  $desc = $collectedClasses.GameMap_cast_ray;
  if ($desc instanceof Array)
    $desc = $desc[1];
  GameMap_cast_ray.prototype = $desc;
  function Camera(ctx, width, height, resolution, spacing, fov, range, lightRange, scale) {
    this.ctx = ctx;
    this.width = width;
    this.height = height;
    this.resolution = resolution;
  ...
  function GameLoop(callback, lastTime) {
    this.callback = callback;
  ...
  function main_closure(player_0, map_1, controls_2, camera_3) {
    this.player_0 = player_0;
    this.map_1 = map_1;
    this.controls_2 = controls_2;
